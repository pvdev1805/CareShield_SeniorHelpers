          ...
          <ul className='space-y-4'>
            {caseNotes.map((note) => (
              <li key={note.id}>
                ...
                >
                  <div className='font-semibold text-purple-700'>Case Note #{note.id}</div>
                  <div className='text-sm text-gray-500'>{formatDate(note.reported_timestamp)}</div>
                  <div className='text-gray-700 mt-1 truncate'>{note.summary}</div>
                </Link>
              </li>
            ))}
          </ul>
